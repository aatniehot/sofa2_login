...
public class AceController {
    ...
    public LoginResponse getLogin(User user){
        String url = "http://localhost:7801/loginapi/v1/login?user="+user.getUsername()+"&pass="+user.getPassword();
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
            HttpEntity<User> entity = new HttpEntity<User>(headers);
            return rest.exchange(url, HttpMethod.GET, entity, LoginResponse.class).getBody();
        }
        public String postLogin(User user) {
            String url = "http://localhost:7801/loginapi/v1/login";
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
